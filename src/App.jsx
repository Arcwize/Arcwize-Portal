...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .login-welcome { font-family: 'Cormorant Garamond', serif; font-size: 2rem; font-weight: 400; color: var(--dark); text-align: center; margin-bottom: 0.4rem; }
  .login-welcome-sub { font-size: 0.6rem; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; color: var(--mid); text-align: center; margin-bottom: 2.5rem; }
  .f-label { display: block; font-size: 0.6rem; font-weight: 600; letter-spacing: 0.2em; text-transform: uppercase; color: var(--mid); margin-bottom: 0.5rem; }
  .f-input { width: 100%; padding: 0.85rem 1rem; background: var(--white); border: 1px solid var(--rule); font-family: 'Jost', sans-serif; font-size: 0.95rem; color: var(--text); outline: none; transition: border-color 0.2s; border-radius: 2px; margin-bottom: 1.25rem; }
  .f-input:focus { border-color: var(--gold); }
  .f-input::placeholder { color: var(--light); }
  .btn-login { width: 100%; padding: 1rem; background: var(--dark); color: var(--white); border: none; font-family: 'Jost', sans-serif; font-size: 0.68rem; font-weight: 600; letter-spacing: 0.25em; text-transform: uppercase; cursor: pointer; transition: background 0.2s; border-radius: 2px; margin-top: 0.5rem; }
  .btn-login:hover { background: var(--dark-mid); }
  .btn-login:disabled { opacity: 0.35; cursor: not-allowed; }
  .f-forgot { text-align: right; margin-bottom: 1rem; margin-top: -0.75rem; }
  .f-forgot button { background: none; border: none; font-family: 'Cormorant Garamond', serif; font-size: 0.9rem; font-style: italic; color: var(--mid); cursor: pointer; }
  .f-forgot button:hover { color: var(--dark); }
  .f-error { font-family: 'Cormorant Garamond', serif; font-size: 0.95rem; font-style: italic; color: var(--red); margin-top: 0.75rem; text-align: center; }
  .reset-sent { text-align: center; font-family: 'Cormorant Garamond', serif; font-size: 1.1rem; font-style: italic; color: var(--mid); padding: 1.5rem; border: 1px solid var(--rule); border-radius: 2px; margin-top: 1rem; line-height: 1.6; }
  ...
  .sidebar-wordmark-sub { font-size: 0.55rem; font-weight: 500; letter-spacing: 0.3em; text-transform: uppercase; color: rgba(255,255,255,0.35); margin-top: 0.3rem; }
  .sidebar-gold-rule { width: 32px; height: 1px; background: var(--gold); margin: 1.5rem 2rem; }
  .sidebar-nav { flex: 1; display: flex; flex-direction: column; gap: 0.15rem; padding: 0 1rem; }
  .nav-item { display: flex; align-items: center; gap: 0.85rem; padding: 0.75rem 1rem; border-radius: 4px; font-size: 0.7rem; font-weight: 500; letter-spacing: 0.12em; text-transform: uppercase; color: rgba(255,255,255,0.5); cursor: pointer; background: none; border: none; width: 100%; text-align: left; transition: all 0.15s; position: relative; }
  .nav-item:hover { color: rgba(255,255,255,0.85); background: rgba(255,255,255,0.05); }
  .nav-item.active { color: var(--white); background: rgba(196,152,90,0.15); }
  .nav-item.active::before { content: ''; position: absolute; left: 0; top: 50%; transform: translateY(-50%); width: 2px; height: 60%; background: var(--gold); border-radius: 0 2px 2px 0; }
  .nav-icon { font-size: 1rem; width: 20px; text-align: center; flex-shrink: 0; }
  .notif-badge { margin-left: auto; background: var(--gold); color: var(--dark); font-size: 0.55rem; font-weight: 700; padding: 0.15rem 0.45rem; border-radius: 10px; min-width: 18px; text-align: center; }
  .sidebar-bottom { padding: 1.5rem 2rem 0; border-top: 1px solid rgba(255,255,255,0.08); margin-top: 1rem; }
  .sidebar-a-mark { font-family: 'Cormorant Garamond', serif; font-size: 2.5rem; font-weight: 300; color: rgba(196,152,90,0.3); line-height: 1; margin-bottom: 0.25rem; }
  .sidebar-tagline { font-family: 'Cormorant Garamond', serif; font-size: 0.85rem; font-style: italic; color: rgba(255,255,255,0.3); line-height: 1.5; }
  .sidebar-micro { font-size: 0.55rem; font-weight: 500; letter-spacing: 0.15em; text-transform: uppercase; color: rgba(255,255,255,0.2); margin-top: 0.3rem; }
  ...
  .page-subtitle { font-size: 0.8rem; color: var(--mid); }
  ...
  .notif-title { font-family: 'Cormorant Garamond', serif; font-size: 1.05rem; font-weight: 500; color: var(--dark); margin-bottom: 0.25rem; }
  ...
  .editor-title { width: 100%; font-family: 'Cormorant Garamond', serif; font-size: 2.8rem; font-weight: 400; color: var(--dark); border: none; border-bottom: 2px solid var(--rule); background: transparent; padding: 0.5rem 0 1rem; outline: none; line-height: 1.1; transition: border-color 0.2s; }
  ...
  .btn-save { font-size: 0.62rem; font-weight: 600; letter-spacing: 0.18em; text-transform: uppercase; color: var(--mid); background: none; border: 1px solid var(--rule); padding: 0.75rem 1.5rem; cursor: pointer; border-radius: 2px; transition: all 0.15s; }
  .btn-save:hover { border-color: var(--dark); color: var(--dark); }
  .btn-submit { font-size: 0.62rem; font-weight: 600; letter-spacing: 0.18em; text-transform: uppercase; color: var(--white); background: var(--dark); border: none; padding: 0.75rem 1.75rem; cursor: pointer; border-radius: 2px; transition: background 0.15s; }
...
function Avatar({ user }) {
  const initials = (user?.user_metadata?.full_name || user?.email || "IN").split(/\s|@/)[0].slice(0, 2).toUpperCase();
  ...
  return (
    <div className="login-page">
      <div className="login-left">
        <div><div className="login-logo">Arcwize</div><div className="login-logo-sub">Portal</div></div>
        <div>
        ...
        <div className="login-form-wrap">
          ...
          {!reset ? (
            <>
              ...
              <input className="f-input" type="email" placeholder="Enter your email" value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === "Enter" && submit()} />
              <label className="f-label">Password</label>
              <input className="f-input" type="password" placeholder="Enter your password" value={pw} onChange={e => setPw(e.target.value)} onKeyDown={e => e.key === "Enter" && submit()} />
              <div className="f-forgot"><button onClick={() => { setReset(true); setErr(""); }}>Forgot password?</button></div>
              <button className="btn-login" onClick={submit} disabled={loading || !email || !pw}>{loading ? "Logging in..." : "Log In"}</button>
            ...
            <>
              ...
              {sent ? (
                <div className="reset-sent">Check your inbox. A reset link has been sent to <strong>{email}</strong>.</div>
              ) : (
                <>
                  <label className="f-label">Email</label>
                  <input className="f-input" type="email" placeholder="Enter your email" value={email} onChange={e => setEmail(e.target.value)} />
                  <button className="btn-login" onClick={sendReset} disabled={loading || !email}>{loading ? "Sending..." : "Send Reset Link"}</button>
                  {err && <div className="f-error">{err}</div>}
                </>
              )}
...
function Sidebar({ tab, setTab, isAdmin, onLogout, unread }) {
  const internItems = [
  ...
  return (
    <div className="sidebar">
      ...
      <button className="logout-btn" onClick={onLogout}><span className="nav-icon">→</span>Log out</button>
...
function ArticleTable({ articles, categories, onEdit, initialTab }) {
  ...
  const filtered = articles
    .filter(a => activeTab === "all" ? true : activeTab === "submitted" ? a.status === "submitted" : a.status === "draft")
    ...
    <div className="table-wrap">
      <div className="table-header">
        <div className="table-tabs">
          {[["all","All Articles"],["submitted","Submitted"],["draft","Drafts"]].map(([val,label]) => (
            <button key={val} className={`table-tab${activeTab===val?" active":""}`} onClick={() => setActiveTab(val)}>{label}</button>
          ))}
        </div>
...
function AdminDashboard({ articles, categories, onEdit }) {
  ...
  return (
    <div className="page">
      <div className="page-title-row"><div><div className="page-title">Dashboard</div><div className="page-subtitle">Managing Editor — Arcwize Editorial Queue</div></div></div>
...
function Notifications({ notifications, onRead }) {
  return (
    <div className="page">
      <div className="page-title-row"><div><div className="page-title">Notifications</div><div className="page-subtitle">Updates on your submitted articles.</div></div></div>
...
function Editor({ user, draft, categories, tags, onSave, onSubmit, onBack, notify }) {
  ...
  const saveToSupabase = async (status) => {
    ...
    if (draft?.id && !draft.id.startsWith("new-")) {
      const { data, error } = await supabase.from("articles").update(payload).eq("id", draft.id).select().single();
  ...
  const doSave = async () => {
    try {
      const saved = await saveToSupabase("draft");
      onSave(saved);
    } catch (e) {
      notify("Failed to save: " + e.message, "err");
    }
  };
  ...
  const doSubmit = async () => {
    ...
    try {
      ...
      notify("Submission error: " + e.message, "err");
    }
    setSubmitting(false);
  };
      ...
      <div className="editor-page">
        ...
        <div className="editor-meta">
          ...
          <div>
            ...
            <textarea className="meta-textarea" rows={2} value={excerpt} onChange={e => setExcerpt(e.target.value)} placeholder="One sentence that introduces your piece..." />
          </div>
        </div>
        ...
        <div className="tags-wrap">
          ...
          <div className="tags-chips">
            {tags.map(t => (
              <button key={t.id} className={`tag-chip${selectedTags.includes(t.id)?" on":""}`} onClick={() => toggleTag(t.id)} type="button">{t.label}</button>
            ))}
          </div>
        </div>
        ...
        <div className="editor-footer">
          ...
          <div className="editor-actions">
            <button className="btn-save" onClick={doSave}>Save Draft</button>
...
export default function App() {
  ...
  const handleSave = (saved) => {
    setArticles(prev => { const i=prev.findIndex(x=>x.id===saved.id); if(i>=0){const n=[...prev];n[i]=saved;return n;} return [saved,...prev]; });
    setDraft(saved);
    notify("Draft saved.");
  };
  ...
  const handleSubmit = (saved) => {
    ...
    setNotifications(prev => [{
      ...
      body: `"${saved.title||"Your article"}" has been submitted and is now in Triston's review queue.`,
      time: fmt(Date.now()),
      unread: true
    }, ...prev]);
    setDraft(null);
  ...
  if (loading) return (
    <><style>{css}</style>
    <div className="loading-screen"><div className="loading-mark">A</div><div className="loading-sub">Loading...</div></div>
    </>
  );
  ...
  return (
    <><style>{css}</style>
    <div className="shell">
      <Sidebar tab={tab} setTab={handleTabChange} isAdmin={isAdmin} onLogout={signOut} unread={unread} />
      <div className="main">
